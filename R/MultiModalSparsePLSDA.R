...
MultiModalSparsePLSDA <- function(data,
    ...
    if (!is.null(keep_features) & !identical(names(keep_features), names(mc$X))) {
        stop("'keep_features' names do not match the assays in the RHS of formula.")
    }
    
    if (!is.matrix(design)) {
        design.type <- match.arg(arg = design, choices = c('full', 'null'))
        design <- .create_design(mc$X, type = design.type, DA = TRUE)
    }
